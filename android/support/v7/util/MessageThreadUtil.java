...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C02441();
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C02451();
            ...
            class C02451 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem next = C04682.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    C04682.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    C04682.this.mQueue.removeMessages(2);
                                    C04682.this.mQueue.removeMessages(3);
                        ...
                        C04682.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
